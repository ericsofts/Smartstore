...
{
    /// <summary>
    /// Defines a set of options used by <see cref="LinkedInHandler"/>.
    /// </summary>
    public class LinkedInOptions : OAuthOptions
    {
        public LinkedInOptions()
        {
            ClaimsIssuer = LinkedInDefaults.Issuer;
            ...
            EmailAddressEndpoint = LinkedInDefaults.EmailAddressEndpoint;

            Scope.Add("r_liteprofile");
            Scope.Add("r_emailaddress");

            ClaimActions.MapJsonKey(ClaimTypes.NameIdentifier, LinkedInConstants.ProfileFields.Id);
            ClaimActions.MapCustomJson(ClaimTypes.Name, user => GetFullName(user));
            ClaimActions.MapCustomJson(ClaimTypes.GivenName, user => GetMultiLocaleString(user, LinkedInConstants.ProfileFields.FirstName));
            ClaimActions.MapCustomJson(ClaimTypes.Surname, user => GetMultiLocaleString(user, LinkedInConstants.ProfileFields.LastName));
            ClaimActions.MapCustomJson(LinkedInConstants.Claims.PictureUrl, user => GetPictureUrls(user)?.LastOrDefault());
            ClaimActions.MapCustomJson(LinkedInConstants.Claims.PictureUrls, user =>
            {
        ...
        {
            LinkedInConstants.ProfileFields.Id,
            LinkedInConstants.ProfileFields.FirstName,
            LinkedInConstants.ProfileFields.LastName,
            LinkedInConstants.EmailAddressField
        };
        ...
        private string GetFullName(JsonElement user)
        {
            ...
            {
                GetMultiLocaleString(user, LinkedInConstants.ProfileFields.FirstName),
                GetMultiLocaleString(user, LinkedInConstants.ProfileFields.LastName),
            };
